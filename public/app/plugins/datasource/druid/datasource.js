...
function (angular, _, moment, dateMath) {
  ...
  module.factory('DruidDatasource', function($q, $http, templateSrv, $timeout, $log) {
    ...
    */
    ...
    DruidDatasource.prototype._topNQuery = function (datasource, intervals, granularity,
      filters, aggregators, postAggregators, threshold, metric, dimension) {
      var query = {
        "queryType": "topN",
        "dataSource": datasource,
        "granularity": granularity,
    ...
    DruidDatasource.prototype._groupByQuery = function (datasource, intervals, granularity,
      filters, aggregators, postAggregators, groupBy, limitSpec) {
      var query = {
        "queryType": "groupBy",
        "dataSource": datasource,
        "granularity": granularity,
    ...
    function dateToMoment(date) {
      ...
      return moment(dateMath.parse(date));
    }
    ...
    function computeGranularity(from, to, maxDataPoints) {
      ...
      */
      ...
      $log.debug("Calculated \"" + granularityEntry[0]  +  "\" granularity ["
        + Math.ceil(intervalSecs/granularityEntry[1].asSeconds()) + " pts]" + " for "
        + (intervalSecs/60).toFixed(0) + " minutes and max of " + maxDataPoints + " data points");
